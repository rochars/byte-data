...
describe('unpack uInt2', function() {   
    ...
    it('should turn 1 2-bit unsigned int to 2 crumb (0s)', function() {
        assert.deepEqual(
            byteData.unpackArray(['00'], byteData.uInt2, 2),
            [0]);
    });
    it('should turn 1 2-bit unsigned int to 2 crumb (0s)', function() {
        assert.deepEqual(
            byteData.unpackArray(['01'], byteData.uInt2, 2),
            [1]);
    });
    it('should turn 1 2-bit unsigned int to 2 crumb (0s)', function() {
        assert.deepEqual(
            byteData.unpackArray(['10'], byteData.uInt2, 2),
            [2]);
    });
    it('should turn 1 2-bit unsigned int to 2 crumb (0s)', function() {
        assert.deepEqual(
    ...
    it('should turn 1 2-bit unsigned int to 1 crumb hex (2)', function() {
        assert.deepEqual(
            byteData.unpackArray(['02'], byteData.uInt2, 16),
            [2]);
    });
    it('should turn 1 2-bit unsigned int to 1 crumb bin (1)', function() {
        assert.deepEqual(
            byteData.unpackArray(['01'], byteData.uInt2, 16),
            [1]);
    });
});
